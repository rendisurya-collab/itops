...
# ---- parser untuk format bahasa natural, misal:
...
def _extract_project_prefix(text: str):
    """Kenali 'TDBU-' atau 'TIC' sebagai kode project. Supaya tidak salah picu
    oleh kata biasa, tanpa tanda '-' hanya diterima kalau HURUF BESAR semua."""
    text = text.strip()
...
def _thread_id_from_update(update: Update):
    """Ambil message_thread_id dari update (kalau pesannya dari dalam topic
    tertentu di grup Forum), supaya balasan bot tetap di topic yang sama."""
...
def _is_allowed_chat(chat_id) -> bool:
    """Akses diizinkan dari: chat personal si pemilik bot, ATAU grup mana pun
    yang terdaftar di TELEGRAM_NOTIFY_TARGETS / TELEGRAM_GROUP_ID -- semua
    anggota grup yang terdaftar otomatis boleh pakai semua command bot."""
...
def restricted(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        if not _is_allowed_chat(update.effective_chat.id):
            await update.effective_message.reply_text(
                "Maaf, bot ini cuma bisa dipakai dari chat/grup yang sudah terdaftar."
            )
...
async def myjira_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # /myjira tanpa argumen (atau argumen lain) -> langsung mulai proses daftar/ganti akun
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Halo! Saya bot bantu logwork ke Jira.\n\n"
        "Perintah yang tersedia:\n"
        "/myjira - daftarkan akun Jira pribadi kamu (wajib sebelum /log dkk)\n"
        "/log - isi logwork baru (tanya berurutan)\n"
        ...
        "/sdtickets [filter] - lihat tiket ServiceDesk Plus (contoh: /sdtickets status:Open group:Network Support)\n"
        ...
        "/run <kata kunci> - langsung eksekusi action/script guidance (skip lihat isi dulu)\n"
        "/addguide - tambah guidance baru\n"
        "/listguide - lihat semua guidance tersimpan\n"
        "/editguide - edit guidance yang sudah ada\n"
        "/delguide - hapus guidance\n"
        "/cancel - batalkan proses yang sedang berjalan\n\n"
        "Tips: ketik langsung kode project-nya (contoh: TDBU- atau TIC) "
        "untuk lihat daftar task-nya tanpa perlu ketik /tasks.\n\n"
        "Tips lain: ketik langsung kalimat pertanyaan/masalah kamu (tanpa command), "
        "bot otomatis munculkan guidance kalau ada yang cocok.\n\n"
        "Tips: /log juga bisa langsung diisi dalam satu kalimat, contoh:\n"
        "/log saya mengerjakan aktifitas di TDBU-10 pada jam 10 pagi, "
        "deskripsi pekerjaanya analyze issue price\n\n"
        "Kalau durasi waktunya belum disebutkan, bot akan tanya belakangan."
    )
...
async def log_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Buang "/log" (dan @namabot kalau ada) di depan, sisanya kalimat bebas (kalau ada)
...
async def advance_log_flow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Cek field mana yang masih kosong, lalu tanyakan itu saja. Kalau semua sudah
    lengkap (misal dari kalimat natural), langsung lanjut ke konfirmasi."""
    d = context.user_data
    msg = update.effective_message
...
async def week_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not jira_client:
        return
    today = dt.datetime.now(TZ).date()
    start = today - dt.timedelta(days=today.weekday())  # Senin
    end = today
    try:
...
def _safe_telegram_text(text: str, limit: int = 3800) -> str:
    """Potong teks kalau melebihi batas aman Telegram (4096 char), supaya
    tidak gagal kirim total dengan error 'Message is too long'."""
    if len(text) <= limit:
        return text
...
def _format_sdp_request_line(req: dict) -> str:
    ...
    return (
        f"• [{req_id}] {subject}\n"
        f"    status: {status} | requester: {requester}"
        f"{desc_line}"
    )
...
def _parse_sdp_filters(text: str) -> dict:
    """Parse 'status:Open group:Network Support' (urutan bebas) jadi dict."""
    filters = {}
...
async def sdtickets_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if group_filter is None and config.SDP_NOTIFY_GROUPS:
        ...
        group_filter = None  # user minta lihat semua group, bypass default
...
async def sdticket_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # description dari SDP biasanya berupa HTML mentah
    ...
    text = (
        ...
        f"Priority: {priority}\n"
        f"Dibuat: {created}\n\n"
        f"Deskripsi:\n{description[:1500] or '(kosong)'}"
    )
    await update.message.reply_text(text)


# ---------------- /export (report Excel per project) ----------------
...
async def delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(f"Gagal menghapus worklog:\n{e}")
    context.user_data.clear()
    return ConversationHandler.END
...
async def send_guidance_entry(update: Update, context: ContextTypes.DEFAULT_TYPE, entry: dict):
    ...
    if attachment and attachment.get("file_id"):
        # Kirim sebagai pesan baru (dengan file), sekalian tutup tombol pilihan kalau ada
        if update.callback_query:
            try:
...
def _action_param_list(action: dict) -> list:
    """Normalisasi config action jadi list parameter, supaya kompatibel dengan
    guidance lama (1 parameter: param_flag/param_type) maupun baru (banyak
    parameter: params: [{flag, type}, ...])."""
...
async def _ask_next_action_param(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Tanyakan parameter berikutnya yang belum diisi, atau lanjut ke konfirmasi
    kalau semua parameter sudah diisi."""
    d = context.user_data
    flags = d["run_flags"]
...
def _filter_item_detail_lines(text: str) -> str:
    """Buang baris detail per-item seperti '[1] Cancel reservasi - ...',
    sisakan ringkasan (Memulai proses, ===== SELESAI =====, Total/Sukses/Gagal, dll)."""
    item_line_re = re.compile(r"^\s*\[\d+\]")
    kept_lines = [line for line in text.splitlines() if not item_line_re.match(line)]
    # rapikan baris kosong berturut-turut jadi maksimal 1
    result_lines = []
    prev_blank = False
...
async def run_action_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for flag_name, info in run_values.items():
        ...
        if info["type"] == "file":
            ...
            cmd.extend(info["value"])  # tiap nilai jadi argumen terpisah -> jadi array di PowerShell
...
async def list_guide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in data:
        ...
        lines.append("")  # baris kosong pemisah antar item
    await update.message.reply_text(_safe_telegram_text("\n".join(lines).rstrip()))


async def guide_free_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Deteksi otomatis kalau pesan bebas (bukan command) cocok dengan salah satu
    guidance tersimpan. Kalau tidak ada yang cocok, diam saja (tidak balas)."""
    if not _is_allowed_chat(update.effective_chat.id):
        return
...
async def addguide_action_flag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lebih dari 1 parameter: tanya dulu cara input datanya nanti.
    keyboard = [
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Error jaringan sementara (timeout, koneksi putus sesaat -- misal karena VPN)
    # bersifat self-recovering (bot otomatis retry), jadi cukup dicatat di log
    # saja, tidak perlu spam notifikasi ke chat.
...
async def check_new_sdp_tickets(context: ContextTypes.DEFAULT_TYPE):
    """Cek tiket baru di group yang dipantau (SDP_NOTIFY_GROUPS), kirim notif
    kalau ada tiket dengan ID lebih baru dari yang terakhir dicek."""
    ...
    if last_seen_id == 0:
        # Pertama kali jalan: cuma set baseline, jangan spam notif tiket lama.
    ...
    for _, req in new_items:
        ...
        text = f"🆕 Tiket baru masuk ({group_name}):\n\n{_format_sdp_request_line(req)}"
...
async def sdreminder_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    state["last_open_reminder_at"] = None  # reset supaya tidak nunggu interval lama sebelumnya
    _save_sdp_notify_state(state)
    await update.message.reply_text(
...
async def check_open_ticket_reminders(context: ContextTypes.DEFAULT_TYPE):
    """Job ini jalan tiap 1 menit sebagai 'detak jantung', tapi cuma benar-benar
    kirim reminder & panggil API kalau sudah waktunya sesuai interval yang diatur
    lewat /sdreminder (disimpan di sdp_notify_state.json, bisa diubah kapan saja)."""
    if not sdp or not config.SDP_NOTIFY_GROUPS:
        return
    ...
    if not open_tickets:
        return  # tidak ada tiket Open, tidak perlu ganggu
...
async def _broadcast_notify(context: ContextTypes.DEFAULT_TYPE, text: str):
    """Kirim satu pesan ke SEMUA target notifikasi (bisa banyak grup/topic sekaligus)."""
...
async def interval_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (config.REMINDER_START_HOUR <= now.hour < config.REMINDER_END_HOUR):
        return  # di luar jam kerja, skip diam-diam
...
def main():
    ...
    # Workaround kompatibilitas untuk Python 3.14+: Python 3.14 tidak lagi
    # otomatis membuat event loop, sedangkan library python-telegram-bot
    # masih mengandalkan perilaku lama tersebut. Kita buat loop-nya secara
    # manual di sini supaya tidak muncul error "no current event loop".
    import asyncio
    ...
    # Deteksi guidance dari teks bebas -- WAJIB paling akhir supaya tidak
    # mengganggu alur command/conversation lain.
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, guide_free_text))

    app.add_error_handler(error_handler)

    # Notifikasi tiket baru ServiceDesk Plus (kalau dikonfigurasi)
    if sdp and config.SDP_NOTIFY_GROUPS:
        ...
        # Reminder tiket Open: job ini "detak" tiap 1 menit, tapi cuma benar-benar
        # kirim reminder sesuai interval yang diatur lewat /sdreminder.
        app.job_queue.run_repeating(
    ...
    # Reminder: berulang tiap interval (kalau REMINDER_INTERVAL_MINUTES diisi),
    # atau sekali sehari di jam tetap seperti biasa.
    if config.REMINDER_INTERVAL_MINUTES > 0:
        app.job_queue.run_repeating(
            interval_reminder,
            interval=dt.timedelta(minutes=config.REMINDER_INTERVAL_MINUTES),
            first=10,  # mulai 10 detik setelah bot jalan
        )
        logger.info(
