...
#        businessUnitCode, transactionNumber, itemCode, itemCode, ..., orderDate
#      Catatan: kolom itemCode boleh muncul lebih dari 1x (berdampingan) untuk
#      transaksi yang punya banyak item. orderDate WAJIB jadi kolom PALING AKHIR.
#   2. Buka PowerShell, masuk ke folder file ini & CSV disimpan, contoh:
#        cd C:\Users\NamaKamu\Downloads
#   3. Jalankan:
#        .\cancel_reservation.ps1 -CsvFile "delete_reservation_debug.csv"
#
...
param(
    [string]$CsvFile = "delete_reservation_debug.csv"
)
...
if (-not (Test-Path $CsvFile)) {
    ...
    Write-Host "Pemakaian: .\cancel_reservation.ps1 -CsvFile 'delete_reservation_debug.csv'"
    exit 1
}
...
#   [0] businessUnitCode, [1] transactionNumber, [2..n-2] itemCode(s), [n-1] orderDate
...
foreach ($row in $parsedRows) {
    ...
    Write-Host "[$total] Cancel reservasi - transactionNumber: $transactionNumber, itemCode(s): $itemCodesDisplay ..." -NoNewline
